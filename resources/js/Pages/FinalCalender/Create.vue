<template>
    <Head>
        <title>Tambah Jadwal Final</title>
        <meta name="description" content="Tambah Jadwal Final" />
    </Head>

    <LoggedInLayout title="Tambah Jadwal Final">
        <Toast class="z-50" />
        ...
        <div>
            <div class="flex justify-between gap-3 mb-2">
                <div>
                    <h1 class="text-2xl font-bold text-primary">Tambah Jadwal Final</h1>
                </div>
            </div>

            <Card class="border">
                <template #title>Form Tambah Jadwal Final</template>
                <template #content>
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <!-- File Upload -->
                            <div class="flex flex-col md:col-span-2">
                                <label for="file" class="mb-1">File (PDF, max 2MB)</label>
...
const submit = () => {
    ...
    form.post(route('final-calender.store'), {
        onSuccess: () => {
            toast.add({
                severity: 'success',
                summary: 'Sukses',
