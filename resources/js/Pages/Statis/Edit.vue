<template>
    <LoggedInLayout title="Edit Halaman Statis">
        ...
        <section class="max-w-8xl p-6 mx-auto bg-gray-50 rounded-md shadow-md dark:bg-gray-800">
            <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Edit Halaman Statis</h2>
...
onMounted(() => {
    ...
    form.judul_seo = props.page.judul_seo;
    form.content = props.page.content;
    form.tgl = props.page.tgl ? new Date(props.page.tgl) : null;
    form.tag = props.page.tag || '';
});

watch(() => form.title, (newTitle) => {
    if (!props.page.judul_seo) {
...
const onFormSubmit = () => {
    form.put(`/admin/statis/${props.page.id}`, {
        onSuccess: () => {
