<template>
    <Head>
        ...
        <meta name="description" content="Edit Permission" />
    </Head>
    <LoggedInLayout title="Edit Izin">
        <Toast />
        ...
        <section class="max-w-8xl p-6 mx-auto bg-gray-50 rounded-md shadow-md dark:bg-gray-800">
            <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Edit Izin</h2>
    ...
    form.put(route('permissions.update', props.permission.id), {
        ...
        onSuccess: () => {
            toast.add({
                severity: 'success',
                summary: 'Berhasil',
                detail: 'Izin berhasil diperbarui',
                life: 3000
            });
        },
        onError: (errors) => {
            toast.add({
                severity: 'error',
                summary: 'Error',
                detail: errors.error || 'Terjadi kesalahan saat memperbarui izin',
                life: 3000
            });
        }
    });
