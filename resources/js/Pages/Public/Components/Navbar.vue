<template>
    <div class="bg-[#0d361d] shadow-sm sticky top-0 z-40 dark:bg-red-900 dark:bg-opacity-50 dark:backdrop-blur-sm">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-end space-x-5 text-xs mx-auto p-2">
            ...
            <a href="https://cdc.uui.ac.id" class="text-gray-100 font-mono" target="_blank">  Career Center </a>
            <Link :href="route('login')" class="text-gray-100 font-mono">Admnistrator</Link>
        </div>
    </div>
    <nav class="bg-white bg-opacity-90 backdrop-blur-xl shadow-sm sticky top-0 z-40 border-b-2 border-green-700 dark:border-green-900 dark:bg-gray-800 dark:bg-opacity-90 dark:backdrop-blur-sm">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
                <img class="h-10"
                     src="/images/logouui.png"/>
                <span class="self-center text-md hidden font-mono lg:block text-green-800 font-semibold whitespace-nowrap dark:text-white">Direktorat Kemahasiswa & Alumni</span>
                <span class="self-center text-md block lg:hidden text-gray-800 font-semibold whitespace-nowrap dark:text-white">DKA</span>
            </a>
            <div class="hidden lg:flex lg:gap-x-5">
                <template v-for="menu in mainmenu" :key="menu.id_main">
                    ...
                    <Link v-if="!menu.submenu || menu.submenu.length === 0"
                        ...
                        class="text-sm font-semibold leading-6 text-gray-900 hover:text-green-600 dark:text-white dark:hover:text-green-400 transition duration-150 ease-in-out">
                        {{ menu.nama_menu }}
                    </Link>
                    <!-- Menu with submenu -->
                    <div v-else class="relative group">
                        <button
                            class="flex items-center text-sm font-semibold leading-6 text-gray-900 hover:text-green-600 dark:text-white dark:hover:text-green-400 transition duration-150 ease-in-out">
                            {{ menu.nama_menu }}
                        ...
                        <div class="absolute left-0 z-10 mt-2 w-48 origin-top-left rounded-md bg-white dark:bg-gray-800 py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none invisible opacity-0 translate-y-2 group-hover:visible group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                            <template v-for="submenu in menu.submenu" :key="submenu.id_sub">
                                ...
                                <Link v-if="isExternalLink(submenu.link_sub)"
                                    ...
                                    target="_blank"
                                    class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-green-600 dark:hover:text-green-400 transition duration-150 ease-in-out">
                                    {{ submenu.nama_sub }}
                                ...
                                <a v-else
                                    ...
                                    :class="{'bg-gray-100 dark:bg-gray-700 text-green-600 dark:text-green-400': route().current(submenu.link_sub)}"
                                    class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-green-600 dark:hover:text-red-400 transition duration-150 ease-in-out">
                                    {{ submenu.nama_sub }}
                                </a>
                            </template>
                        </div>
